...
class AddressInputValidator {
  ...
  static const String _allowedCharactersMessage =
  ...
  static String? validateHouseNumber(String? value) {
    final v = value?.trim() ?? '';
    if (v.isEmpty) return 'Please enter your house, flat, or address name';
    if (v.length > 50) {
      return 'Address details are too long. Please keep them under 50 characters.';
    }
    if (!_allowedChars.hasMatch(v)) {
      return _allowedCharactersMessage;
    }
    if (looksLikeGibberish(v)) {
      return 'Please enter a valid house, flat, or address name';
    }
    return null;
  }
  ...
  static String? validateFloor(String? value) {
    ...
    if (v.length > 15) {
      return 'Floor details are too long. Please keep them under 15 characters.';
    }
    if (!_allowedChars.hasMatch(v)) return _allowedCharactersMessage;
    ...
    return 'Enter a floor like 2, G, Ground or Basement.';
  }
  ...
  static String? validateTowerBlock(String? value) {
    ...
    if (v.isEmpty) return null;
    if (v.length > 40) {
      return 'Tower or block details are too long. Please keep them under 40 characters.';
    }
    if (!_allowedChars.hasMatch(v)) return _allowedCharactersMessage;
    if (looksLikeGibberish(v)) {
    ...
    return 'Enter tower or block details like A, Tower 2 or Block B.';
  }

  static String? validateLandmark(String? value) {
    final v = value?.trim() ?? '';
    if (v.isEmpty) return null;
    if (v.length < 3) return 'Please enter a clearer nearby landmark.';
    if (v.length > 80) {
      return 'Landmark details are too long. Please keep them under 80 characters.';
    }
    if (!_allowedChars.hasMatch(v)) return _allowedCharactersMessage;
    if (looksLikeGibberish(v)) {
    ...
    return 'Add a clear landmark like near City Park or opposite HDFC Bank.';
  }
}
